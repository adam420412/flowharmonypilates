...
VALUES ('unpaid_payment_minutes', '2'::jsonb, 'Ile minut klient ma na opłacenie rezerwacji awansowanej z listy rezerwowej (test: 2, docelowo 1440)')
ON CONFLICT (key) DO NOTHING;

CREATE OR REPLACE FUNCTION public.booking_is_paid(_booking_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (SELECT 1 FROM public.package_redemptions WHERE booking_id = _booking_id)
      OR EXISTS (SELECT 1 FROM public.payments WHERE booking_id = _booking_id AND status = 'paid');
$$;

REVOKE ALL ON FUNCTION public.booking_is_paid(uuid) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.booking_is_paid(uuid) TO authenticated, service_role;

-- cancel_booking: awansowana osoba dostaje termin na opłatę
...
-- Automatyczne odwoływanie nieopłaconych rezerwacji
...
BEGIN
  ...
  LOOP
    ...
    UPDATE public.bookings SET
      status = 'confirmed',
      payment_due_at = now() + make_interval(mins => v_minutes),
      updated_at = now()
    WHERE id = (
      SELECT id FROM public.bookings
...
REVOKE ALL ON FUNCTION public.expire_unpaid_bookings() FROM public, anon;
GRANT EXECUTE ON FUNCTION public.expire_unpaid_bookings() TO authenticated, service_role;
