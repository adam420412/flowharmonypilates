ALTER TABLE public.bookings ADD COLUMN IF NOT EXISTS confirm_due_at timestamptz;

UPDATE public.app_settings SET value = '"Flow & Harmony"'::jsonb, updated_at = now() WHERE key = 'studio_name';
...
BEGIN
  ...
  SET payment_due_at = NULL,
      confirm_due_at = now() + make_interval(mins => v_minutes),
      updated_at = now()
...
CREATE OR REPLACE FUNCTION public.confirm_attendance(_booking_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF v_b.user_id <> v_caller AND NOT public.has_role(v_caller, 'admin') THEN
  ...
  UPDATE public.bookings SET confirm_due_at = NULL, updated_at = now() WHERE id = _booking_id;
...
BEGIN
  ...
  FOR r IN
    ...
    WHERE b.status = 'confirmed'
      AND (
        (b.payment_due_at IS NOT NULL AND b.payment_due_at < now() AND NOT public.booking_is_paid(b.id))
        OR (b.confirm_due_at IS NOT NULL AND b.confirm_due_at < now())
      )
  LOOP
    UPDATE public.bookings SET status = 'cancelled', payment_due_at = NULL, confirm_due_at = NULL, updated_at = now() WHERE id = r.id;
...
BEGIN
  ...
  UPDATE public.bookings SET status = 'cancelled', payment_due_at = NULL, confirm_due_at = NULL, updated_at = now() WHERE id = _booking_id;
