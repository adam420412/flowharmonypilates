CREATE OR REPLACE FUNCTION public.set_payment_due_on_booking()
RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
  ...
  IF NEW.payment_due_at IS NOT NULL THEN
    RETURN NEW;
  END IF;

  SELECT price_grosz INTO v_price FROM public.classes WHERE id = NEW.class_id;
  IF COALESCE(v_price, 0) <= 0 THEN
    RETURN NEW;
  END IF;

  SELECT (value::text)::int INTO v_minutes FROM public.app_settings WHERE key = 'unpaid_payment_minutes';
  NEW.payment_due_at := now() + make_interval(mins => COALESCE(v_minutes, 1440));
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_set_payment_due_on_booking ON public.bookings;
CREATE TRIGGER trg_set_payment_due_on_booking
BEFORE INSERT OR UPDATE ON public.bookings
FOR EACH ROW EXECUTE FUNCTION public.set_payment_due_on_booking();
...
BEGIN
  ...
  UPDATE public.bookings SET payment_due_at = NULL WHERE id = v_booking_id;
